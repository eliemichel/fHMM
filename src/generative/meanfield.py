...
def meanfield(logpi_l, logA_l, C_m, W_l, Y_m, Nit):
    ...
    for n in range(Nit):
        ...
        for m in range(M):
            ...
            # create residual vector and diagonal matrix
            Yres_v = np.copy(Y0_v)
            ...
            logthetaint0m_v = (
                np.dot(np.transpose(W_l[m]),
                       np.dot(np.linalg.inv(C_m), Yres_v)) - 1/2*diag_v +
                np.dot(np.transpose(logA_l[m]), theta_l[1][m]) +
                np.reshape(logpi_l[m], (K, 1)))
            logthetaint0m_v = (logthetaint0m_v -
                               sp_misc.logsumexp(logthetaint0m_v))
            theta_l[0][m] = np.exp(logthetaint0m_v)
        ...
        for t in range(1, T):
            ...
            for m in range(M):
                ...
                logthetainttm_v = (logthetainttm_v -
                                   sp_misc.logsumexp(logthetainttm_v))
                theta_l[t][m] = np.exp(logthetainttm_v)
    ...
    for t in range(1, T+1):
        for m in range(M):
            for k1 in range(K):
                for k2 in range(K):
                    EmixT_l[t][m, k1, k2] = (
                        theta_l[t-1][m][k1] * theta_l[t][m][k2])
