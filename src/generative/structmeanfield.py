# -*- coding: utf-8 -*-
"""
Created on Tue Jan  3 11:58:48 2017
...
def structmeanfield(logpi_l, logA_l, C_m, W_l, Y_m, Nit):
    ...
    for n in range(Nit):
        ...
        #Computing and storing the deltas
        ...
        for t in range(T+1):
            ...
            for m in range(M):
                ...
                for n in range(M):
                    if n != m:
                        Yres_v += - np.dot(W_l[n], reshape(E_l[t][n,:], (K,1)))
                h_l[t][m] = (exp(np.dot(np.transpose(W_l[m]), 
                          np.dot(np.linalg.inv(C_m), Yres_v)) -1/2*delta[m]))
...
def structmeanfield_E(logpi_l, logA_l, h_l):
    ...
    for m in range(M):
        for k in range(K):
            muf_l[1][m][k] = logh_l[1][m][k] + (sumprodlog( logh_l[0][m] +
            reshape(logpi_l[m], (K,1)) + reshape(logA_l[m][:,k], (K,1))))
        
    for m in range(M):
        for k in range(K):
            mub_l[T-1][m][k] = (sumprodlog(logh_l[T][m] + 
            reshape(logA_l[m][k,:], (K,1))))
    
    #Forward-backward message passing
    for t in range(1,T):
        for m in range(M):
            for k in range(K):
                muf_l[t+1][m][k] = logh_l[t+1][m][k] + (sumprodlog(
                reshape(logA_l[m][:,k], (K,1)) + np.copy(muf_l[t][m])))
                
                mub_l[T-t-1][m][k] = (sumprodlog(logh_l[T-t][m] + 
                reshape(logA_l[m][k,:], (K,1)) + np.copy(mub_l[T-t][m])))
    
    #Filling E_l
    for t in range(1,T+1):
        for m in range(M):
            Eint_v = muf_l[t][m] + mub_l[t][m]
            Eint_v += -sumprodlog(Eint_v)
            E_l[t][m,:] = reshape(exp(Eint_v), (K,))
            
    for m in range(M):
        Eint_v = (logh_l[0][m] + reshape(logpi_l[m], (K,1)) + muf_l[0][m] + 
        mub_l[0][m])
        Eint_v += -sumprodlog(Eint_v)
        E_l[0][m,:] = reshape(exp(Eint_v), (K,))
        ...
        for m1 in range(M):
            for m2 in range(M):
                if m2 == m1:
                    ...
                    EmixM_l[t][m1,m2,:,:] = (np.dot(reshape(E_l[t][m1,:], (K,1)), 
                    np.transpose(reshape(E_l[t][m2,:], (K,1))),))
    ...
    for t in range(2,T+1):
        for m in range(M):
            ...
            for k1 in range(K):
                for k2 in range(K):
                    Eint_m[k1,k2] = (logh_l[t][m][k2] + logA_l[m][k1,k2] + 
                    muf_l[t-1][m][k1]+ mub_l[t][m][k2])
            Eint_m += -sumprodlog(Eint_m)
            EmixT_l[t][m,:,:] = exp(Eint_m)
            
    for m in range(M):
        Eint_m = np.zeros((K,K))
        for k1 in range(K):
            for k2 in range(K):
                Eint_m[k1,k2] = (logh_l[1][m][k2] + logpi_l[m][k1] + 
                logh_l[0][m][k1] + logA_l[m][k1,k2] + muf_l[0][m][k1] +
                mub_l[1][m][k2])
        Eint_m += -sumprodlog(Eint_m)
        EmixT_l[1][m,:,:] = exp(Eint_m)
