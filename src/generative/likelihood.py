...
def loglikelihood(pi_l, A_l, C_m, W_l, Y_m, E_l, EmixM_l, EmixT_l):
    ...
    # initialize the logarithm of the parameters and the loglikelihood
    logpi_l = np.log(pi_l)
    logA_l = np.log(A_l)
    L = 0

    # compute L for the 0 states
    for m in range(M):
        L += sum(logpi_l[m] * E_l[0][m, :])

    # add the dependencies between the hidden variables
    ...
    for t in range(T+1):
        ...
        L += -1/2 * np.dot(np.dot(np.transpose(Y_v), np.linalg.inv(C_m)),
                           Y_v)[0, 0]
        for m in range(M):
            L += np.dot(np.transpose(Y_v), np.dot(
                np.linalg.inv(C_m),
                np.dot(W_l[m], np.transpose(np.array([E_l[t][m, :]])))))[0, 0]
            for n in range(M):
                L += -1/2 * np.trace(np.dot(
                    np.transpose(W_l[m]),
                    np.dot(np.linalg.inv(C_m), np.dot(
                        W_l[n],
                        EmixM_l[t][n, m, :, :]))))
